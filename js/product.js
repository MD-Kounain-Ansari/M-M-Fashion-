...
function renderProductGallery(images) {
  ...
  productMainImage.alt = `${product.name} view 1`;
  ...
  images.forEach((image, index) => {
    ...
    thumbnail.addEventListener("click", () => {
      ...
      productMainImage.alt = `${product.name} view ${index + 1}`;
...
reviewsToggle?.addEventListener("click", () => {
  const isOpen = reviewsToggle.getAttribute("aria-expanded") === "true";

  reviewsToggle.setAttribute("aria-expanded", String(!isOpen));

  reviewsContent.hidden = isOpen;

  reviewsToggle.querySelector(".reviews-arrow").textContent = isOpen
    ? "+"
    : "−";
});
...
function renderProduct() {
  ...
  if (productOldPrice) {
    if (product.oldPrice) {
      productOldPrice.textContent = `₹${product.oldPrice.toLocaleString(
        "en-IN",
      )}`;
  ...
  if (productMainImage) {
    productMainImage.src = product.images?.[0] || product.image;
  ...
  if (productFit) {
    productFit.textContent = product.fit || "";
  }
  ...
  if (productPayment) {
    productPayment.textContent = product.payment || "";
  }
}

const productAccordions = document.querySelectorAll(".product-accordion-btn");

productAccordions.forEach((button) => {
  button.addEventListener("click", () => {
    const content = button.nextElementSibling;

    const icon = button.querySelector(".product-accordion-icon");
...
function renderProductColors() {
  ...
  if (!product.colors || product.colors.length === 0) {
    ...
    if (selectedColor) {
      selectedColor.textContent = "Default";
    }
  ...
  product.colors.forEach((color, index) => {
    ...
    button.className = "product-color-btn";
    ...
    button.addEventListener("click", () => {
      document.querySelectorAll(".product-color-btn").forEach((item) => {
        item.classList.remove("active");
      });
      ...
      if (productMainImage && color.image) {
        productMainImage.src = color.image;
      }
    });
...
function renderProductSizes() {
  ...
  product.sizes.forEach((size, index) => {
    ...
    button.className = "product-size-btn";

    button.textContent = size;

    button.addEventListener("click", () => {
      document.querySelectorAll(".product-size-btn").forEach((item) => {
        item.classList.remove("active");
      });
...
function renderSimilarProducts() {
  ...
  const ids = Array.isArray(product.similarProducts)
    ? product.similarProducts
    : [];

  ids.forEach((id) => {
    const relatedProduct = window.products?.find((item) => item.id === id);

    if (!relatedProduct) {
      return;
    }

    similarProductsGrid.appendChild(createRelatedProductCard(relatedProduct));
  });
}
...
function renderAlsoBought() {
  ...
  const ids = Array.isArray(product.alsoBought) ? product.alsoBought : [];
  ...
  ids.forEach((id) => {
    ...
    alsoBoughtGrid.appendChild(createRelatedProductCard(relatedProduct));
  });
}
